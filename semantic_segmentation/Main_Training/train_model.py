...
def main_loop(batch_size=config.batch_size, model_type='', tensorboard=True):
    ...
    if model_type == 'UCTransNet':
        ...
        pretrained_UNet_model_path = "nets/model_0.7924695423572304_0.7901398768876351.pth"
        pretrained_UNet = torch.load(pretrained_UNet_model_path, map_location='cuda')

        model2_dict = model.state_dict()
        pretrained_UNet = pretrained_UNet.items()

        state_dict = {list(model2_dict.keys())[0]:torch.cat((list(pretrained_UNet)[0][1],list(pretrained_UNet)[0][1]), dim=0)}
