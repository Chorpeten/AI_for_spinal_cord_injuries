...
def vis_and_save_heatmap(model, input_img, img_RGB, labs, vis_save_path, dice_pred, dice_ens):
    ...
    dice_pred_tmp, iou_tmp = show_image_with_dice(predict_save, labs, save_path=vis_save_path+'_predict'+'.png')
    return dice_pred_tmp, iou_tmp


test_session = "Test_session_12.03_20h56"
if __name__ == '__main__':
    ...
    if config.task_name == "GlaS":
        ...
        model_path = "./nets/best_model-" + model_type + ".pth.tar"

    elif config.task_name == "SpinalCordInjury":
        test_num = len(os.listdir('./datasets/Feature_preparation/img/'))
        model_type = config.model_name
        # model_path = "./MoNuSeg/"+model_type+"/"+test_session+"/models/best_model-"+model_type+".pth.tar"
        model_path = "./SpinalCordInjury/UCTransNet_pretrain/Test_session_12.03_20h56/models/best_model-UCTransNet_pretrain.pth.tar"

    save_path  = config.task_name +'/'+ model_type +'/' + test_session + '/'
    vis_path = './datasets/Feature_preparation/prediction/'
    ...
    test_folder_pth = './datasets/Feature_preparation/'
    test_dataset = ImageToImage2D(test_folder_pth, tf_test, image_size=config.img_size)
    test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False)
    # print(test_dataset.images_list)
